...
export default function CustomerDetailPage() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error("Errore", {
        description:
          error.response?.data?.message || "Impossibile aggiornare il cliente",
      });
    },
  });
        ...
        <Badge
          className={
            customer.type === "company"
              ? "bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800 font-medium text-xs border"
              : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 font-medium text-xs border"
          }
        >
          {customer.type === "company" ? "Azienda" : "Privato"}
                      ...
                      >
                        ...
                        <TableCell className="text-right">
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/quotes/${quote.id}`}>Visualizza</Link>
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))}

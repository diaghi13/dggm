...
export function ProjectServicesSection({
  ...
  const [selectedService, setSelectedService] = useState<App.Data.ProjectMaterialData | null>(null);

  // Fetch services from API (only service type)
  const { data: services = [], refetch } = useQuery({
    queryKey: ['project-services', projectId],
    queryFn: () => projectMaterialsApi.getAll(projectId, { product_type: 'service' }),
    enabled: !!projectId,
  });

  // Calculate statistics
  const extraServices = services.filter((s: App.Data.ProjectMaterialData) => s.is_extra);
  const stats = {
    totalPlanned: services.reduce((sum: number, s: App.Data.ProjectMaterialData) =>
      sum + (Number(s.planned_quantity || 0) * Number(s.planned_unit_cost || 0)), 0),
    totalActual: services.reduce((sum: number, s: App.Data.ProjectMaterialData) =>
      sum + (Number(s.used_quantity || 0) * Number(s.actual_unit_cost || 0)), 0),
    totalServices: services.length,
    completed: services.filter((s: App.Data.ProjectMaterialData) =>
      Number(s.used_quantity || 0) >= Number(s.planned_quantity || 0)).length,
    extraServices: extraServices.length,
    extraCost: extraServices.reduce((sum: number, s: App.Data.ProjectMaterialData) =>
      sum + (Number(s.used_quantity || 0) * Number(s.actual_unit_cost || 0)), 0),
  };
  ...
  const getUsageProgress = (service: App.Data.ProjectMaterialData) => {
    const planned = Number(service.planned_quantity || 0);
    if (planned === 0) return 0;
    return (Number(service.used_quantity || 0) / planned) * 100;
  };

  const getCostVariance = (service: App.Data.ProjectMaterialData) => {
    ...
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Gestisci i servizi (ore lavoro, noleggi, trasporti) per questo progetto
              </CardDescription>
            </div>
          ...
          {services.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Inizia ad aggiungere servizi a questo progetto
              </p>
              <Button onClick={() => setIsAddDialogOpen(true)}>
                <Plus className="mr-2 h-4 w-4" />
            ...
            <div className="rounded-md border">
              <Table>
                ...
                <TableBody>
                  {services.map((service: App.Data.ProjectMaterialData) => {
                    ...
                    return (
                      <TableRow key={service.id}>
                        ...
                        <TableCell className="text-right">
                          {service.planned_quantity} {service.product?.unit}
                        </TableCell>
                        <TableCell className="text-right">
                          {service.used_quantity} {service.product?.unit}
                        </TableCell>
                        <TableCell>
                          <div className="w-24">
      ...
      <LogMaterialUsageDialog
        ...
        material={selectedService ? {
          ...
          product: selectedService.product ?? null,
          planned_quantity: Number(selectedService.planned_quantity ?? 0),
          used_quantity: Number(selectedService.used_quantity ?? 0),
          remaining_quantity: selectedService.remaining_quantity ?? undefined,
          planned_unit_cost: selectedService.planned_unit_cost ?? null,
        } : null}

...
import { ProjectDdtsTable } from '@/app/(dashboard)/projects/_components/project-ddts-table';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function ProjectDocumentsSection({
    ...
    async (acceptedFiles: File[], collectionKey: string) => {
      for (const file of acceptedFiles) {
        ...
        try {
          ...
          await projectsApi.uploadMedia(projectId, formData);

          toast.success('File caricato', {
            description: `${file.name} è stato aggiunto al progetto`,
          });
  ...
  const handleDownload = useCallback(async (mediaItem: Media) => {
    try {
      const blob = await projectsApi.downloadMedia(mediaItem.id);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      await projectsApi.deleteMedia(mediaToDelete.id);
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeCollection} onValueChange={setActiveCollection}>
        ...
        <TabsContent value="ddts">
          <ProjectDdtsTable projectId={projectId} />
        </TabsContent>

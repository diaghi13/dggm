...
interface ProjectDdtsTableProps {
  projectId: number;
}

export function ProjectDdtsTable({ projectId }: ProjectDdtsTableProps) {
  const queryClient = useQueryClient();
  const [statusFilter, setStatusFilter] = useState<string>("all");

  // Fetch DDTs
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["project-ddts", projectId, statusFilter],
    queryFn: () =>
      projectsApi.getDdts(
        projectId,
  ...
  const confirmMutation = useMutation({
    mutationFn: (ddtId: number) => projectsApi.confirmDdt(projectId, ddtId),
    onSuccess: () => {
      toast.success("DDT confermato", {
        description:
          "Il DDT è stato confermato e i materiali sono stati scaricati dal magazzino.",
      });
    ...
    onError: (error: any) => {
      toast.error("Errore", {
        description:
          error.response?.data?.message || "Impossibile confermare il DDT",
      });
    },
  });
  ...
  const getTypeBadge = (type: string) => {
    const config = {
      ...
      rental_out: {
        label: "Noleggio Uscita",
        color: "bg-orange-100 text-orange-700",
      },
      rental_return: {
        label: "Noleggio Rientro",
      ...
      {ddts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center border rounded-lg">
          ...
          <p className="text-sm text-muted-foreground">
            Non ci sono DDT per questo progetto
          </p>
        </div>
      ) : (
              ...
              {ddts.map((ddt: any) => (
                <TableRow key={ddt.id}>
                  <TableCell className="font-medium">
                    {ddt.code || ddt.ddt_number}
                  </TableCell>
                  <TableCell>{getTypeBadge(ddt.type)}</TableCell>
                  <TableCell>
                    {ddt.ddt_date
                      ? format(new Date(ddt.ddt_date), "dd MMM yyyy", {
                          locale: it,
                        })
                      : "-"}
                  </TableCell>

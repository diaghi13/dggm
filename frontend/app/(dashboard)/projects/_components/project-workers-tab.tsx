...
import { ProjectWorkerStatusBadge } from '@/app/(dashboard)/projects/_components/project-worker-status-badge';
import { ProjectRoleBadges } from '@/app/(dashboard)/projects/_components/project-role-badge';
import { AssignWorkerDialog } from '@/app/(dashboard)/projects/_components/assign-worker-dialog';
import { ManageWorkerRolesDialog } from '@/components/manage-worker-roles-dialog';
import { ProjectWorker, ProjectWorkerStatus } from '@/lib/types';
import {
  UserPlus,
  MoreVertical,
  CheckCircle2,
...
export function ProjectWorkersTab({ projectId }: ProjectWorkersTabProps) {
  ...
  const acceptMutation = useMutation({
    mutationFn: (projectWorkerId: number) => projectWorkersApi.acceptAssignment(projectWorkerId),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: (projectWorkerId: number) => projectWorkersApi.rejectAssignment(projectWorkerId),
    onSuccess: () => {
  ...
  const changeStatusMutation = useMutation({
    mutationFn: ({ projectWorkerId, status }: { projectWorkerId: number; status: any }) =>
      projectWorkersApi.changeStatus(projectWorkerId, { status }),
    onSuccess: () => {
  ...
  const removeMutation = useMutation({
    mutationFn: (projectWorkerId: number) => projectWorkersApi.removeWorker(projectWorkerId),
    onSuccess: () => {
      toast.success('Lavoratore rimosso dal progetto');
      queryClient.invalidateQueries({ queryKey: ['project-workers', projectId] });
    },
    onError: () => {
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as ProjectWorkerStatus | 'all')}>
                <SelectTrigger>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Squadra Progetto</CardTitle>
                  ...
                  <TableRow key={worker.id}>
                    ...
                    <TableCell>
                      {worker.roles && worker.roles.length > 0 ? (
                        <ProjectRoleBadges roles={worker.roles} />
                      ) : (

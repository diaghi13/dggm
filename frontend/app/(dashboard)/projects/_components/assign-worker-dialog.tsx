  ...
  const assignMutation = useMutation({
    mutationFn: (data: AssignWorkerFormData) => {
      ...
      if (isEditMode && assignment) {
        ...
        return projectWorkersApi.updateAssignment(assignment.id, updatePayload);
      } else {
        return projectWorkersApi.assignWorker(projectId, payload);
      }
    },
    onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditMode ? 'Modifica Assegnazione' : 'Assegna Lavoratore al Progetto'}</DialogTitle>
          <DialogDescription>
            {isEditMode
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="use-rate-override" className="cursor-pointer">
                  Sovrascrivi tariffa per questo progetto
                </Label>
              </div>

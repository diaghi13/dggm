...
export const createStockMovementsColumns = (): ColumnDef<StockMovement>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <StockMovementTypeBadge
        type={row.original.type as App.Enums.StockMovementType}
      />
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div
          className={`text-right font-medium ${isPositive ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
        >
          {isPositive ? "+" : "-"}
          {Number(movement.quantity).toFixed(2)} {movement.material?.unit}
        </div>
      );
    },
  },
        ...
        return (
          <div className="text-sm">
            <div className="flex items-center gap-1">
              ...
              <Link
                href={`/warehouses/${movement.from_warehouse?.id}`}
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <Link
                href={`/warehouses/${movement.to_warehouse?.id}`}
                className="hover:underline text-blue-600 dark:text-blue-400"
              >
        ...
        return (
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm text-slate-700 dark:text-slate-300">
              {movement.supplier.company_name}
            </span>
          </div>
        );
      }
